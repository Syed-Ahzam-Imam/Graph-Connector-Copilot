...
{
    Console.WriteLine(
        $"Current connection: {(currentConnection == null ? "NONE" : currentConnection.Name)}\n"
    );
...
static DateTime GetLastUploadTime()
{
    ...
    {
        return DateTime.Parse(File.ReadAllText("lastuploadtime.bin")).ToUniversalTime();
    }
...
{
    var connectionId =
        PromptForInput("Enter a unique ID for the new connection (3-32 characters)", true)
        ?? "ConnectionId";
    var connectionName =
        PromptForInput("Enter a name for the new connection", true) ?? "ConnectionName";
    ...
    {
        Console.WriteLine(
            $"Error creating connection: {odataError.ResponseStatusCode}: {odataError.Error?.Code} {odataError.Error?.Message}"
        );
        return null;
    }
}
...
async Task DeleteCurrentConnectionAsync(ExternalConnection? connection)
{
    ...
    {
        Console.WriteLine(
            $"Error deleting connection: {odataError.ResponseStatusCode}: {odataError.Error?.Code} {odataError.Error?.Message}"
        );
    }
}
...
async Task GetSchemaAsync()
{
    ...
    {
        Console.WriteLine(
            $"Error getting schema: {odataError.ResponseStatusCode}: {odataError.Error?.Code} {odataError.Error?.Message}"
        );
    }
}
...
async Task UpdateItemsFromDatabaseAsync(bool uploadModifiedOnly, string? tenantId)
{
    ...
    if (uploadModifiedOnly)
    {
        ...
        partsToUpload = partsDb
            .Parts.Where(p => EF.Property<DateTime>(p, "LastUpdated") > lastUploadTime)
            .ToList();

        partsToDelete = partsDb
            .Parts.IgnoreQueryFilters()
            .Where(p =>
    ...
    Console.WriteLine(
        $"Processing {partsToUpload.Count} add/updates, {partsToDelete.Count} deletes."
    );
    var success = true;
    ...
    foreach (var part in partsToUpload)
    {
        ...
        catch (ODataError odataError)
        {
            ...
            Console.WriteLine(
                $"Error: {odataError.ResponseStatusCode}: {odataError.Error?.Code} {odataError.Error?.Message}"
            );
        }
    }
